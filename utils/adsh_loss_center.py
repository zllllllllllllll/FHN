...
from utils.fuzzy_clustering import FuzzyCMeans
...
class ADSHLoss(nn.Module):
    ...
    def forward(self, u, V, S, V_omega, labels, train_label):
        # output为batch输出torch, V为数据库哈希码numpy, S, V[batch_ind.cpu().numpy(), :], database_labels, train_label
        batch_size = u.size(0)
        u = u.tanh()
        u = u.cuda()
        V_omega = Variable(torch.from_numpy(V_omega).type(torch.FloatTensor).cuda())  # V_omega cuda torch float32
        if self.is_single == 1:
            category_labels = np.argmax(train_label, axis=1)  # 每一行最大元素的索引
            #  根据类别筛选哈希码
            V_per_list = []
