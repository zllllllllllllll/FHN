...
class FuzzyCMeans(BaseCluster):
    def __init__(self, n_cluster, scale=1., m='auto', error=1e-5, tol_iter=100, verbose=0):
        """
        Implantation of fuzzy c-means
        :param n_cluster: number of clusters
        :param m: fuzzy index
        :param error: max error for u_old - u_new to break the iteration
        :param tol_iter: total iteration number    200
    ...
    def fit(self, X, y=None):
        ...
        self.variance_ = np.fmax(self.variance_, np.finfo(np.float32).eps)
        return self
    ...
    def _cmean_update(self, X, U):
        ...
        dist = np.fmax(dist, np.finfo(np.float64).eps)
        U = self.normalize_column(1.0 / dist)
